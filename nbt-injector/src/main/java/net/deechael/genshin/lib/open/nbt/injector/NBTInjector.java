...
import net.deechael.genshin.lib.open.nbt.NBTContainer;
import net.deechael.genshin.lib.open.nbt.NBTEntity;
import net.deechael.genshin.lib.open.nbt.NBTReflectionUtil;
import net.deechael.genshin.lib.open.nbt.NBTTileEntity;
import net.deechael.genshin.lib.open.nbt.NbtApiException;
import net.deechael.genshin.lib.open.nbt.utils.MinecraftVersion;
import net.deechael.genshin.lib.open.nbt.utils.nmsmappings.ClassWrapper;
import net.deechael.genshin.lib.open.nbt.utils.nmsmappings.ObjectCreator;
...
public class NBTInjector {
	...
	public static void inject() {
		...
		if(MinecraftVersion.getVersion() == MinecraftVersion.MC1_7_R4 || MinecraftVersion.isAtLeastVersion(MinecraftVersion.MC1_16_R1)) {
			logger.warning("[Genshin Craft Lib - NBT] The NBT-Injector is not compatibel with this Minecraft Version! For 1.16+ please use the persistent storage API.");
			return;
		}
		isInjected = true;
		try {
			ClassPool classPool = ClassPool.getDefault();
			logger.info("[Genshin Craft Lib - NBT] Injecting Entity classes...");
			...
			logger.info("[Genshin Craft Lib - NBT] Injecting Tile Entity classes...");
			if (MinecraftVersion.getVersion().getVersionId() <= MinecraftVersion.MC1_10_R1.getVersionId()) {
